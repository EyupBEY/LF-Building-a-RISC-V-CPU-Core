...
\TLV
   ...
   // FIELDS
   //Extracting Fields, no need $func7 since we wont use.
   ...
   //Determine when these fields are valid, but $opcode which is always valid.
   ...
   $imm[31:0] = $is_i_instr ? { {21{$instr[31]}}, $instr[30:20] } :
                ...
                32'b0;  // Default
   //Verify in WAVEFORM and VIZ that the value of $imm corresponds to the instructions in the test program. This will test your understanding of binary, decimal and hexadecimal. For example, the ADDI, x12, x0, 1010 instruction shows the immediate value in binary, VIZ should represent the value in decimal as i[10], and WAVEFORM should show a hexadecimal "a".
   
   // SUBSET_INSTRS
   $dec_bits[10:0] = {$instr[30], $funct3, $opcode}; // For convenience, concatenate the relevant fields into a single bit vector signal.
   $is_beq = $dec_bits ==? 11'bx_000_1100011; //Underscore is just helpful to read us.
